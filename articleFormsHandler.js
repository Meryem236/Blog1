...
export default class articleFormsHandler {
    ...
    processArtEditFrmData(event) {
        ...
        const updatedAuthor = loggedInUser || currentAuthor; // Use logged-in user or keep current author
    
        const articleData = {
            title: this.formElements.namedItem("title").value.trim(),
            content: this.formElements.namedItem("content").value.trim(),
            author: updatedAuthor, // Set the author based on login state
    ...
    processNewArticleFrmData(event) {
        ...
        const uniqueTag = 'Fabricea'; // Ensure this is updated
        if (articleData.tags) {
        ...
        fetch(`${this.serverUrl}/articles`, postReqSettings)
            ...
            .then(responseJSON => {
                ...
                // Create a new article object to add to the UI
                const newArticle = {
                    id: responseJSON.id, // Assuming the response contains the new article ID
                    title: responseJSON.title,
                    content: responseJSON.content,
                    author: responseJSON.author,
                    imageLink: responseJSON.imageLink,
                    tags: responseJSON.tags,
                    detailLink: `#article/${responseJSON.id}/0/0` // Adjust as necessary
                };
    
                // Update the articles list in the UI without fetching again
                const articlesContainer = document.getElementById("router-view"); // Assuming this is where articles are displayed
                const articlesTemplate = document.getElementById("template-articles").innerHTML;
    
                // Render the new article directly
                articlesContainer.innerHTML = Mustache.render(articlesTemplate, {
                    articles: [newArticle], // Wrap in an array to render as a list
                    showPrevious: false,
                    showNext: false,
                    previousOffset: 0,
                    nextOffset: 0,
                    totalCount: 1 // Adjust as necessary
                });
...
document.getElementById('btAddArticle').addEventListener('click', function() {
    ...
    if (formContainer.classList.contains('hiddenElm')) {
        ...
        .replace('{{author}}', loggedInUser || '') // Pre-fill author if logged in
        .replace('{{title}}', '')
        .replace('{{imageLink}}', '')
        .replace('{{content}}', '')
        .replace('{{tags}}', '')
