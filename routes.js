...
export default[
    ...
    },
    {
        ...
        getTemplate: (targetElm) => {
            ...
            form.elements["opnElm"].value = loggedInUser || ""; // Set to logged-in user's name or empty
            form.elements["nopOpn"].value = loggedInUserEmail || ""; // Set to logged-in user's email or empty
    ...
    function reqListener() {
      ...
      if (this.status == 200) {
          ...
          if (forEdit) {
            ...
            if (loggedInUser) {
                authorField.value = loggedInUser; // Set to logged-in user
            } else {
                authorField.value = ""; // Clear the field if logged out
            }
        
            // Assign form functionality
            ...
            window.artFrmHandler.assignFormAndArticle("articleForm", "hiddenElm", artIdFromHash, offsetFromHash, totalCountFromHash);
                ...
                if (addCommentBtn) {
                    addCommentBtn.onclick = function () {
                        ...
                        // Pre-fill the author field with the logged-in user's name
